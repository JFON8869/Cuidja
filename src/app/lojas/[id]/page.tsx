...
export default function StorePage() {
  ...
  } = useCollection<Product>(servicesQuery);
  ...
  const isLoading = isStoreLoading || isProductsLoading || isServicesLoading;
  ...
  return (
    <div className="relative mx-auto flex min-h-[100dvh] max-w-sm flex-col bg-background shadow-2xl">
      <header className="relative h-32">
         <Image
            src={store.bannerUrl || 'https://picsum.photos/seed/store-banner/600/200'}
        ...
        <div className="absolute top-2 left-2">
          <Button variant="ghost" size="icon" className="rounded-full bg-black/50 text-white hover:bg-black/70 hover:text-white" asChild>
            <Link href={initialCategory ? `/categorias/${initialCategory}` : '/home'}>
              <ArrowLeft />
            </Link>
          </Button>
        </div>
      ...
      <main className="flex-1">
        <div className="-mt-16 px-4">
            <div className="flex items-end gap-4">
                 <Image
                    src={store.logoUrl || genericLogo?.imageUrl || `https://picsum.photos/seed/${store.id}/128`}
                ...
                />
                 <Badge variant={isOpen ? "default" : "secondary"} className={isOpen ? 'border-green-600 bg-green-500 text-white' : 'bg-muted text-muted-foreground'}>
        ...
        <div className="mt-6">
           <Tabs defaultValue={defaultTab} className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="products" disabled={!hasProducts}>Produtos</TabsTrigger>
                    <TabsTrigger value="services" disabled={!hasServices}>Serviços</TabsTrigger>
                </TabsList>

...
*   **Ações:**
    ...
    2.  **Validação de Input no Backend:** Implementar uma camada de validação do lado do servidor (usando Cloud Functions) para todas as operações de escrita no Firestore, como uma segunda linha de defesa contra dados maliciosos.
...
*   **Ações:**
    1.  **Implementar Busca com Algolia/Typesense:** Substituir a busca nativa do Firestore (que tem limitações) por um serviço de busca dedicado. Isso permite filtros complexos, busca por texto completo e resultados instantâneos.
    ...
    3.  **Cache Inteligente de Dados:** Implementar uma estratégia de cache no lado do cliente (SWR ou React Query) para dados que não mudam com frequência (ex: lista de categorias, detalhes de lojas populares).
...
### **Sprint 4: Notificações e Engajamento em Tempo Real (Médio Prazo)**

*   **Objetivo Técnico:** Fornecer aos vendedores e compradores ferramentas de engajamento em tempo real.
*   **Justificativa de Negócio:** Uma comunicação rápida e um vendedor engajado são a espinha dorsal do marketplace.
*   **Ações:**
    1.  **Notificações Push (FCM):** Implementar Firebase Cloud Messaging para alertar vendedores sobre novos pedidos e compradores sobre atualizações de status, mesmo com o app fechado.
    2.  **Sistema de Avaliações:** Construir a estrutura para que clientes possam avaliar produtos e lojas após a conclusão de um pedido.
    3.  **Favoritos:** Permitir que usuários salvem suas lojas e produtos preferidos para fácil acesso.
